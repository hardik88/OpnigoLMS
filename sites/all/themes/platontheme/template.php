 ...
 */
function platontheme_theme() {
  return array(
    'platontheme__header' => array(
      'variables' => array('page' => NULL),
      'template' => 'templates/platontheme--header',
    ),
    'platontheme__site_content' => array(
      'variables' => array('page' => NULL),
      'template' => 'templates/platontheme--site-content',
    ),
    'platontheme__site_content__first_sidebar' => array(
      'variables' => array('page' => NULL),
      'template' => 'templates/platontheme--site-content--first-sidebar',
    ),
    'platontheme__site_content__second_sidebar' => array(
      'variables' => array('page' => NULL),
      'template' => 'templates/platontheme--site-content--second-sidebar',
    ),
    'platontheme__footer' => array(
      'variables' => array('page' => NULL),
      'template' => 'templates/platontheme--footer',
    ),
    'platontheme__main_navigation__item' => array(
      'variables' => array('item' => NULL),
      'template' => 'templates/platontheme--main-navigation--item',
    ),
    'platontheme__main_navigation__row' => array(
      'variables' => array('items' => array()),
      'template' => 'templates/platontheme--main-navigation--row',
    ),
    'platontheme__message' => array(
      'variables' => array('type' => 'status', 'heading' => NULL, 'messages' => array()),
      'template' => 'templates/platontheme--message',
    ),
    'platontheme__app_list' => array(
      'variables' => array('items' => NULL),
      'template' => 'templates/platontheme--app-list',
    ),
  );
}
 ...
 */
function platontheme_theme_registry_alter(&$registry) {
 ...
 */
function platontheme_process_html(&$vars) {
 ...
 */
function platontheme_preprocess_html(&$vars) {
  ...
  if (empty($vars['user']->uid) 
   && $vars['is_front']
   && $settings['platontheme_use_home_page_markup']) {

    $vars['classes_array'][] = 'platontheme-use-slider';
  }

  if (!user_is_logged_in() && $vars['is_front'] && !theme_get_setting('platontheme_menu_show_for_anonymous')) {
    $vars['classes_array'][] = 'platontheme-hide-anonymous-menu';
  }
}
 ...
 */
function platontheme_process_page(&$vars) {
  ...
  $vars['platontheme__header'] = theme('platontheme__header', $vars);
  $vars['platontheme__site_content__first_sidebar'] = theme('platontheme__site_content__first_sidebar', $vars);
  $vars['platontheme__site_content__second_sidebar'] = theme('platontheme__site_content__second_sidebar', $vars);
  $vars['platontheme__site_content'] = theme('platontheme__site_content', $vars);
  $vars['platontheme__footer'] = theme('platontheme__footer', $vars);
}

/**
 * Implements hook_preprocess_page().
 */
function platontheme_preprocess_page(&$vars) {

  $path = drupal_get_path('theme', 'platontheme');
  _platontheme_inject_css_override();
  ...
  if (theme_get_setting('platontheme_use_header_background') && theme_get_setting('platontheme_header_image_path')) {
    if (!isset($site_header_attributes['style'])) {
      $site_header_attributes['style'] = '';
    }
    $site_header_attributes['style'] .= 'background-image: url("' . file_create_url('public://' . theme_get_setting('platontheme_header_image_path')) . '");';
  }
  ...
  $vars['main_navigation'] = _platontheme_get_main_navigation();
    ...
    if (!empty($group['gid'])) {
      ...
      foreach (array(
        "node/{$group['gid']}" => array(
          'title' => "",
          'class' => 'platontheme-og-context-view-tab platontheme-og-context-home-tab',
        ),
        "node/{$group['gid']}/edit" => array(
          'class' => 'platontheme-og-context-view-tab platontheme-og-context-settings-tab',
          'query' => array('destination' => current_path()),
        ),
        "node/{$group['gid']}/group" => array(
          'class' => 'platontheme-og-context-view-tab platontheme-og-context-users-tab',
        ),
        "node/{$group['gid']}/tools" => array(
          'class' => 'platontheme-og-context-view-tab platontheme-og-context-tools-tab',
        ),
        // Ajout du menu trier les cours - Cédric - 10.09.2014
        "node/{$group['gid']}/sort_courses" => array(
          'class' => 'platontheme-og-context-view-tab platontheme-og-context-sort-tab',
        ),
      ) as $path => $override) {
        ...
        foreach (opigno_get_node_tools($groups) as $tool) {
              ...
              if (!empty($link) && $link['access']) {
                  ...
                  if (!empty($tool['machine_name'])) {
                    $link['options']['attributes']['class'][] = $link['localized_options']['attributes']['class'][] = 'platontheme-og-context-view-tab platontheme-og-context-'. $tool['machine_name'] .'-tab';
                  }
                  if (!empty($tool['query'])) {
                    if (!isset($link['options']['query'])) {
                      $link['options']['query'] = array();
      ...
      if (!empty($tabs)) {
        if (isset($vars['node'])&&!platontheme_display_tabs($vars['node']))
        {
  ...
  if (function_exists('privatemsg_unread_count')) {
    ...
    drupal_add_js(array('platontheme' => array('unreadMessages' => $unread)), 'setting');
  }
  ...
  if (empty($vars['user']->uid) 
   ...
   && $settings['platontheme_use_home_page_markup']) {
    ...
    foreach ($settings as $key => $value) {
      ...
      if (is_array($value) && $key != 'platontheme_home_page_markup') {

        if (!empty($value['platontheme_home_page_markup']['value'])) {
          $markupValue = $value['platontheme_home_page_markup']['value'];
        } else {
          $markupValue = $value['platontheme_home_page_markup_wrapper']['platontheme_home_page_markup']['value'];
        }

        if (!empty($value['platontheme_home_page_markup']['format'])) {
          $markupFormat = $value['platontheme_home_page_markup']['format'];
        } else {
          $markupFormat = $value['platontheme_home_page_markup_wrapper']['platontheme_home_page_markup']['format'];
        }

        if (!empty($value['platontheme_home_page_markup']['background'])) {
          $markupBackground = $value['platontheme_home_page_markup']['background'];
        } else {
          $markupBackground = $value['platontheme_home_page_image_path'];
        }
  ...
  if (variable_get('platontheme_group_style', 1) == 1) {
    if (isset($vars['group_state'])) {
      $vars['group_state'] = platontheme_render_group_state($vars['group_state']);
    }
  }
}

function platontheme_render_group_state($var) {
  $empty = TRUE;
 ...
 */
function platontheme_preprocess_node(&$vars) {
  ...
  if (module_exists('quiz') && $vars['node']->type == 'quiz') {
    global $user;
    $vars['passed_quiz'] = _platontheme_check_user_passed_quiz($vars['node'], $user);
  }
}
 ...
 */
function platontheme_preprocess_field(&$vars, $hook) {
 ...
 */
function platontheme_status_messages($vars) {
  ...
  foreach (drupal_get_messages($display) as $type => $messages) {
    $output .= theme('platontheme__message', array('type' => $type, 'heading' => !empty($status_heading[$type]) ? $status_heading[$type] : NULL, 'messages' => $messages));
  }
  return $output;
}

function platontheme_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function platontheme_form_quiz_question_answering_form_alter(&$form, $form_state) {
 ...
 */
function platontheme_apps_list($vars) {
  $items = array();
  ...
  return theme('platontheme__app_list', array('items' => $items));
}

/**
 * Helper function to create the main navigation.
 * Returns the rendered main navigation.
 * @see platontheme_theme().
 *
 * @return string
 */
function _platontheme_get_main_navigation() {
  $html = '';
  $items = _platontheme_get_main_navigation_items();
  $items_per_col = 2;
  ...
  for ($i = 0, $rows = ceil(count($items) / $items_per_col); $i < $rows; $i++) {
    ...
    while ($j && ($item = array_shift($items))) {
      if (!empty($item['#href'])) {
        $row_html .= theme('platontheme__main_navigation__item', array('item' => $item));
        $j--;
      }
    }
    // If any items were rendered, wrap them in a row.
    if (!empty($row_html)) {
      $html .= theme('platontheme__main_navigation__row', array('items' => $row_html));
    }
  }
 ...
 * @return array
 */
function _platontheme_get_main_navigation_items() {
  $source = @theme_get_setting('platontheme_menu_source');
  if (!empty($source)) {
 ...
 * @return string
 */
function platontheme_get_group_title($nid)
{
 ...
 * @return object|false
 */
function _platontheme_get_css_override_file() {
  $fid = theme_get_setting('platontheme_css_override_fid');
  if (!empty($fid))
  {
    $file = file_load($fid);
 ...
 * @return int
 */
function _platontheme_store_css_override_file($content)
{
  global $user;
  ...
  // Create a new file.
  $filename = uniqid(md5($content)) . '_platontheme_override.css';
  $uri = "public://$filename";
  $f = fopen($uri, 'w');
 ...
 * @return string
 */
function _platontheme_get_css_override_file_content() {
  if ($file = _platontheme_get_css_override_file()) {
 ...
 */
function _platontheme_inject_css_override() {
  if ($file = _platontheme_get_css_override_file()) {
 ...
 */
function _platontheme_color_remove_white($filepath) {
 ...
 * @return bool
 */
function _platontheme_check_user_passed_quiz($node, $user)
{
  $res = db_query('
 ...
 */
function platontheme_display_tabs($node) {
  $types = array();
